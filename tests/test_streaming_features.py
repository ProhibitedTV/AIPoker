...
class StreamingFeatureTests(unittest.TestCase):
    ...
    def test_overlay_serves_browser_page_and_json_state(self):
        ...
        try:
            ...
            self.assertEqual(state["health"]["components"]["casino_program"], "ready")
            ...
            self.assertIn("casino", state)
            self.assertEqual(state["casino"]["schema_version"], 1)
            self.assertEqual(state["casino"]["active_game"], "poker")
            self.assertIn("program_block", state["casino"])
            self.assertIn("fictional_bankrolls", state["casino"])
            self.assertIn("responsible_label", state["casino"])
            ...
            self.assertIn("lower_third", state["presentation"])
            self.assertEqual(state["presentation"]["lower_third"]["schema_version"], 1)
            self.assertIn("modules", state["presentation"]["lower_third"])
            self.assertIn("ticker_events", state["presentation"]["lower_third"])
            self.assertIn("ticker_items", state["presentation"]["lower_third"])
            ...
            self.assertIn("scene_state", state["presentation"])
            self.assertEqual(state["presentation"]["scene_state"]["schema_version"], 1)
            self.assertIn(state["presentation"]["scene_state"]["state"], {"standby", "live_hand", "break", "table_reset"})
            self.assertIn("bartender", state["presentation"])
            self.assertEqual(state["presentation"]["bartender"]["schema_version"], 1)
            self.assertIn("voice_cue", state["presentation"]["bartender"])
            ...
            self.assertIn("venue", state["presentation"])
            self.assertEqual(state["presentation"]["venue"]["schema_version"], 1)
            self.assertIn("header_label", state["presentation"]["venue"])
            self.assertIn("safe_label", state["presentation"]["venue"])
            ...
            self.assertIn("nickname", state["players"][0]["profile"])
            self.assertIn("archetype", state["players"][0]["profile"])
            self.assertIn("model_name", state["players"][0]["profile"])
            self.assertIn("attitude", state["players"][0]["profile"])
            self.assertIn("reputation", state["players"][0]["profile"])
            self.assertIn("moment_lines", state["players"][0]["profile"])
            ...
            self.assertIn("personaCardMarkup", html)
            self.assertIn("personaMoment", html)
            self.assertIn("persona-meta", html)
            ...
            self.assertIn('id="venueChip"', html)
            self.assertIn('id="venueStatus"', html)
            self.assertIn("venueThemeOn", html)
            self.assertIn("q.get('venue')==='0'", html)
            ...
            self.assertIn('id="casinoRoom"', html)
            self.assertIn("renderCasinoRoom", html)
            self.assertIn("Fictional bankroll ladder", html)
            self.assertIn('id="streamScene"', html)
            self.assertIn("renderStreamScene", html)
            self.assertIn('data-stream-scene="live_hand"', html)
            self.assertIn("sceneSweep", html)
            ...
            self.assertIn('id="lowerThird"', html)
            self.assertIn('data-lower-third-mode="table"', html)
            self.assertIn("renderLowerThird", html)
            self.assertIn("renderHauntTicker", html)
            self.assertIn("haunt-event", html)
            self.assertIn("haunt-ticker", html)
            self.assertIn("lowerthird", html)
            self.assertIn("deskOn=q.get('desk')==='1'", html)
            self.assertIn("hudMode=q.get('hud')||'production'", html)
            ...
            self.assertIn("lastBartenderVoiceKey", html)
            ...
            self.assertIn("casino_program", stream_info)
            self.assertEqual(stream_info["casino_program"]["active_game"], "poker")
    ...
    def test_overlay_director_query_overrides_and_visual_debug(self):
        ...
        try:
            ...
            self.assertIn("venueThemeDefault='on'==='on'", html)
            self.assertIn("venue-theme-off", html)
            self.assertIn("visual-debug", html)
            self.assertIn("director-off", html)
            self.assertIn("lowerThirdOn=q.get('lowerthird')==='0'?false:true", html)
            self.assertIn("document.body.classList.toggle('hud-minimal',minimalHud)", html)
    ...
    def test_overlay_browser_audio_and_music_can_be_enabled_for_obs_source(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIn('data-director="on" data-director-mode="table" data-lower-third-mode="table" data-stream-scene="live_hand" data-intensity="0" class=" "', muted_html)
