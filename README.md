...
The OBS scene uses a seated casino-table layout for 2–6 players, with a dealer/deck tray, visible pot chips, per-seat stack chips, committed wager chips, and a TV-style lower third for the current beat, plain-English headline, rotating info module, and recent-action crawl. It explains each street in plain English, expands dealer and blind abbreviations, identifies the acting player, describes the immediate choice, and labels equity as “chance to win.” A five-step hand tracker, chip-leader marker, big-pot and all-in tension cues, newcomer-friendly statistics, winner takeover, and restrained celebration animation keep the story legible even for viewers who have never played poker. The production HUD includes an OBS-readability pass for Twitch compression: larger center cards, stronger rank/suit contrast, bigger pot/street labels, calmer folded seats, and reduced idle-seat microcopy so the hand remains readable at 50% preview scale. These layers are event-driven, so idle polling never replays a card, chip, action, or award animation.

Player identity is presentation metadata on top of the poker state. The default cast now carries scalable OBS avatar archetypes, sigils, short taglines, configurable persona cards, nicknames, archetypes, model-name labels, attitudes, table reputations, and optional all-in/win/loss/fold/idle moment lines. Active and spotlight seats show this “casino regular” texture while idle seats stay compact in the production HUD, and each card still keeps the actual model-source chip visible for Ollama/fallback transparency. The OBS card faces use a stronger holo-card treatment with cyan/magenta suit glow, circuit-like borders, scanlines, and Night-City-style rim lighting. The scene also layers in an original rainy megacity skyline, diegetic neon signs, and holographic board labeling behind the felt. A presentation-only venue skin pulls public AI-lounge data into compact header chips, table labels, city signs, and footer copy so the room feels like a back-alley AI casino without covering the hand. Use `overlay_venue_theme_enabled: false`, `--no-venue-theme`, or `?venue=0` on the browser source to disable that skin while iterating. The same cyan/magenta glass treatment now extends across table, decision, big-pot, all-in, showdown, recap, winner, bumper, compact, and reduced-motion modes while preserving high-contrast cards, pot, actions, odds, and safety labels.

The broadcast director layer is enabled by default. `/state.presentation` tells the OBS page when to use table, decision, big-pot, all-in, showdown, or recap presentation. It also publishes `presentation.scene_state` so the same OBS browser source can show polished standby/loading, intermission, and table-reset slates without flashing blank screens. Use `?director=0` for a plain table source and `?visual_debug=1` to show safe-area/director labels while tuning OBS. `python scripts/visual_smoke.py` generates and checks deterministic visual fixtures for every director and scene-state mode.

The OBS browser source is the canonical viewer-facing show surface; the Qt window is the local control room. To keep the stream understandable without requiring poker literacy or reading every panel, `/state.presentation` includes a showrunner beat with a one-line viewer focus, large non-reader labels, optional host voice cues, and `presentation.lower_third` for the main TV-broadcast information package. The lower third rotates public-state modules for pot/cost, equity, stacks, model health, casino program status, lounge mood, and chat prompts while the hand remains visually unobstructed. Its `ticker_events` feed turns recent actions, all-ins, street reveals, showdown results, eliminations, table resets, and low-priority Night City flavor into clipped severity-tagged OBS crawl pills; poker-critical events always outrank ambience during live hands. The legacy side broadcast desk is hidden during normal lower-third mode; use `?desk=1` to show it for debugging, `?hud=full` to restore dense seat diagnostics, or `?lowerthird=0` for a plain-table capture. Use `overlay_showrunner_enabled`, `overlay_voice_cues_enabled`, `overlay_voice_cooldown_ms`, `overlay_non_reader_mode`, `overlay_rotation_enabled`, `overlay_rotation_interval_ms`, and `overlay_narration_enabled` in config, or URL overrides like `?showrunner=0`, `?voice=0`, `?nonreader=0`, `?rotation=0`, `?rotation_ms=12000`, and `?narration=1` / `?tts=1`, to tune the TV-style guidance and optional browser speech narration.

Mira-7, the default AI bartender, adds short data-driven room interjections for pre-hand, all-in, river, showdown, bust-out, table-reset, and idle beats. These lines surface in the same lower-third ticker and publish `presentation.bartender.voice_cue` so OBS can speak restrained bartender TTS through the browser source when voice cues are enabled; cue IDs rotate by hand context to avoid repeating the same line every poll. When the optional voice service is enabled, bartender cues use the same cached `/voice/*.wav` and operator-supplied RVC conversion path as host/player table-talk clips, then gracefully fall back to browser speech if a clip is not ready. New room characters or line packs can be added in `presentation.py` without changing the poker engine.
