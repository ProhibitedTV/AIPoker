...
Player identity is presentation metadata on top of the poker state. The default cast now carries scalable OBS avatar archetypes, sigils, and short taglines, rendered as cyberpunk neon medallions inside each seat card. The OBS card faces use a holo-card treatment with cyan/magenta suit glow and circuit-like borders, while live decision panels compress into lower-obstruction glass when the action matters. This gives the table an underground AI-casino feel while keeping the cards, action, pot, and odds as the highest-contrast elements on screen.
