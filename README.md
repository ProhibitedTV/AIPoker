...
The OBS browser source is the canonical viewer-facing show surface; the Qt window is the local control room. To keep the stream understandable without requiring poker literacy or reading every panel, `/state.presentation` includes a showrunner beat with a one-line viewer focus, large non-reader labels, and optional host voice cues for decisions, all-ins, winners, lounge moments, format changes, and intermissions. The overlay also includes a broadcast-desk rotator over the live felt. It cycles public-state cards for table action, equity, stack standings, model health, program/story context, lounge atmosphere, and sanitized table talk while the hand continues underneath. Use `overlay_showrunner_enabled`, `overlay_voice_cues_enabled`, `overlay_voice_cooldown_ms`, `overlay_non_reader_mode`, `overlay_rotation_enabled`, `overlay_rotation_interval_ms`, and `overlay_narration_enabled` in config, or URL overrides like `?showrunner=0`, `?voice=0`, `?nonreader=0`, `?rotation=0`, `?rotation_ms=12000`, and `?narration=1` / `?tts=1`, to tune the TV-style guidance and optional browser speech narration.
